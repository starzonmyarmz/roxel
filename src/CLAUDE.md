...
**`PointerState`** carries `anchor` (`StrokeAnchor` locks build plane axis to one of X/Y/Z during drag) and `last_placed` (endpoint for `line3d` 3D Bresenham — fills jump gaps; Shift+click runs one-shot `line3d` from `last_placed` to target).
...
`color_space.rs` — sRGB u8 storage everywhere. `ColorSpace` enum (`Hex`/`Rgb`/`Hsl`/`Hsb`/`Oklch`) selects the picker-popup readout; the active space is a preference (Preferences → Color → Format), not an inspector control. Conversions: `parse_hex`, `rgb_to_hsl`/`hsl_to_rgb`, `rgb_to_hsb`/`hsb_to_rgb`, `rgb_to_oklch`/`oklch_to_rgb`. OKLCH path reuses `mesh::srgb_to_linear`/`linear_to_srgb` so the voxel-color pipeline and the inspector roundtrip match. sRGB roundtrip is within ±1/255.
